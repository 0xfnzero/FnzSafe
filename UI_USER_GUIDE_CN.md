...
新创建或通过助记词导入的钱包使用 v3 助记词 Keystore：主密文保存助记词，导入后可以重新派生 Solana 和 EVM 账户。通过私钥创建或导入的钱包使用私钥 Keystore，只能恢复该私钥对应的账户。钱包列表会明确显示 Keystore 类型。
...
导入助记词 Keystore 会恢复并校验其 Solana 地址，同时从同一助记词恢复默认 EVM 派生账户。导入私钥 Keystore 只恢复文件中记录的单个账户；私钥无法反推出助记词。
